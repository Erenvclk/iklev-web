import { getBoardMembers, getBackupBoardMembers, getSupervisoryBoardMembers } from '@/lib/strapi';
...
const BOARD_SECTIONS = [
  {
    ...
    fetcher: getBoardMembers,
  },
  {
    key: 'backup',
    heading: 'Yedek Yönetim Kurulu',
    fetcher: getBackupBoardMembers,
  },
  {
    key: 'supervisory',
    heading: 'Denetim Kurulu',
    fetcher: getSupervisoryBoardMembers,
  },
] as const;

function MemberTable({ members }: { members: BoardMember[] }) {
  if (members.length === 0) {
    return (
...
export default async function YonetimKurullariPage() {
  ...
  const sections = [
    { heading: 'Yönetim Kurulu',       members: mainRes?.data ?? [] },
    { heading: 'Yedek Yönetim Kurulu', members: backupRes?.data ?? [] },
    { heading: 'Denetim Kurulu',        members: supervisoryRes?.data ?? [] },
  ];
  ...
  return (
    <div className="min-h-screen bg-stone-50">
      ...
      <div className="max-w-4xl mx-auto px-6 py-12 flex flex-col gap-10">
        {sections.map(({ heading, members }) => (
          <section key={heading}>
            <h2 className="text-lg font-semibold text-stone-700 mb-3">{heading}</h2>
